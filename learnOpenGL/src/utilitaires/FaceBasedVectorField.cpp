#include "FaceBasedVectorField.h"

FaceBasedVectorField::FaceBasedVectorField(vector<Vertex> sommets, vector<unsigned int> indices){
...
void FaceBasedVectorField::draw(Shader &shader){
    glBindVertexArray(VAO);
    glDrawElements(GL_TRIANGLES,static_cast<unsigned int>(indices.size()),GL_UNSIGNED_INT,0);
    glBindVertexArray(0);
}

void FaceBasedVectorField::FreeResources(){
    glDeleteVertexArrays(1,&VAO);
    glDeleteBuffers(1,&VBO);
    glDeleteBuffers(1,&EBO);
}

void FaceBasedVectorField::setUpMesh(){
...
void FaceBasedVectorField::computeVectorField(){
